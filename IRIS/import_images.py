...
Here, Ke's data (R Ke, Nat. Methods, 2013) is employed as the major data structure in our software. In this data structure, 
barcodes are composed of 4 types of pseudo-color, representing the A, T, C, G bases. In addition there's a DAPI background.

In the type of Eng's data, each image is composed of 4 channels, of which, the first 3 channels represents blobs by 3 pseudo-colors and the last one is background. Then, each continuous 4 images are made a Round, also
...
def decode_data_Eng(f_cycles):
    """
    For parsing data generated by the technique described in Eng et al. Nature Methods (2013) and Eng et al. Nature (2019)
