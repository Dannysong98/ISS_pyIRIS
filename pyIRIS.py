...
Description:    This software is used to transform the fluorescent signal of in situ sequencing (ISS) into base
                sequences (barcode sequences), as well as the quality of each base and their coordinates in the background
                image.
...
if __name__ == '__main__':
    """
    This is the entry function of our software, which can be split three modules, images import, blob detection
    and base connection.
    
    The images importing module is compatible with two types of data, which are generated from the technologies by 
    Ke et al. 2013 and Chen et al. 2015, respectively.
    
    Our software control the data importing by two options following the main command, of which, the '--ke' means to 
    process the data generated by in situ sequencing, and the '--chen' means processing MERFISH data, with its
    optimized parameters.
    """
    if len(argv) > 2 and ('--ke' in argv[1] or '--chen' in argv[1]):
        cycle_stack = []
        ...
        print('Invalid image group\nUSAGE:  ' + argv[0] + ' <--ke|--chen> <image group>', file=stderr)
